...
new #[Layout('components.layouts.admin-app')] class extends Component {
    #[Validate('required|string', as: 'グループ名')]
    public string $name = '';
    #[Validate('required|string', as: 'グループ説明')]
    ...
    public function addSkill(int $skillId): void
    {
        ...
        if ($skill && !in_array($skill->id, $this->showSkills)) {
        ...
        try {
            DB::transaction(function () {
                ...
                foreach ($this->showSkills as $skill) {
                    $group->skills()->attach(Skill::where('name', $skill)->first()->id);
                }
            });

            // 入力値をリセット
            $this->reset(['name', 'description', 'showSkills', 'search']);

            // 成功時の処理
            session()->flash('message', 'グループが正常に登録されました。');
        } catch (\Exception $e) {
            // エラー時の処理
            session()->flash('error', 'グループの登録に失敗しました。' . $e->getMessage());
        }
    }
}; ?>
...
<div>
    ...
    <div class="rounded-lg border-gray-700 border p-8 bg-gray-850 shadow-xl">
        <h2 class="text-2xl font-bold text-white mb-6 border-b border-gray-700 pb-2">グループ登録</h2>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <button wire:click.prevent="createGroup"
