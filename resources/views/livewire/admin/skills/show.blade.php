...
<div>
    <div class="bg-gray-800 shadow-sm rounded-lg p-6">
        ...
        <div class="mb-8 p-4 bg-gray-700 rounded-lg">
            <h2 class="text-xl font-semibold text-white mb-4">基本情報</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-sm font-medium text-gray-400">スキル名</p>
                    <p class="text-lg font-medium text-white">{{ $skill->name }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-400">作成日時</p>
                    <p class="text-base text-white">{{ $skill->created_at->format('Y/m/d H:i') }}</p>
                </div>
                <div class="md:col-span-2">
                    <p class="text-sm font-medium text-gray-400">説明</p>
                    <p class="text-base text-white">{{ $skill->description }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- スキルレベル情報 -->
        <div class="mb-8">
            <h2 class="text-xl font-semibold text-white mb-4">スキルレベル</h2>
                                    ...
                                    <div class="space-y-2">
                                        @foreach ($level->effects as $effect)
                                            <div class="flex items-center">
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-700 text-gray-200 mr-2">
                                                    {{ $effectStatuses[$effect->effect_status] ?? $effect->effect_status }}
                                                </span>
                                                @if ($effect->effect_type === 'add')
                                                    <span class="text-green-400">+{{ $effect->effect_value }}</span>
                                                ...
                                                @else
                                                    <span class="text-gray-400">効果なし</span>
                                                @endif
        ...
        <!-- 使用武器 -->
        <div>
            <h2 class="text-xl font-semibold text-white mb-4">このスキルを持つ武器
                ({{ $skill->weapons->count() }})</h2>

            @if ($weapons->isEmpty())
                <div class="bg-gray-700 p-4 rounded-md text-center">
                    <p class="text-gray-400">このスキルを持つ武器はありません</p>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-700">
                            ...
                            @foreach ($weapons as $weapon)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <div class="h-10 w-10 rounded bg-gray-700 flex items-center justify-center">
